    ...
    def generatePath(self, m, n):
        ...
        def findLongestPath(visited, i, j, x, y, dist, path):
            ...
            if isValid(i+1, j) and isSafe(visited, i+1, j):
                findLongestPath(visited, i+1, j, x, y, dist+1, path+[(i, j)])
            if isValid(i-1, j) and isSafe(visited, i-1, j):
                findLongestPath(visited, i-1, j, x, y, dist+1, path+[(i, j)])
            if isValid(i, j+1) and isSafe(visited, i, j+1):
                findLongestPath(visited, i, j+1, x, y, dist+1, path+[(i, j)])
            if isValid(i, j-1) and isSafe(visited, i, j-1):
                findLongestPath(visited, i, j-1, x, y, dist+1, path+[(i, j)])

            visited.remove((i, j))

        findLongestPath(set(), 0, 0, 0, n-1, 0, [])
        self.longest_path = self.longest_path[randint(
            0, len(self.longest_path)-1)]
...
def game():
    ...
    G = g()
    path = G.generatePath(SCREEN_WIDTH//20, SCREEN_WIDTH//20)
    cur = 0
